...
class SubtaskHelper extends Base
{
    ...
    public function renderGitCommitField(array $values, array $errors = array(), array $attributes = array())
    {
        $attributes = array_merge(array('tabindex="15"'), $attributes);

        $html = $this->helper->form->label(t('Git Commit estimate'), 'time_estimated');
        $html .= $this->helper->form->text('git_commit', $values, $errors, $attributes);
